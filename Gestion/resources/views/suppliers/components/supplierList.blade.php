    ...
    @foreach ($suppliers as $supplier)
      <div class="row supplier-table mx-0 py-1">
        <div class="col-1 d-flex px-1">
          @switch($supplier->latestNonModifiedStatus()->status)
            ...
            @case('removed')
              ...
              <div class="text-center supplier-list-table-text text-removed ps-1">{{__('global.removed')}}</div>
              @break
        ...
        <div class="col-1 d-flex justify-content-between">
          ...
          <input type="checkbox" name="suppliers[]" class="supplier-select-check" value={{$supplier->id}}>
        </div>
      </div>
    @endforeach
  </form>
