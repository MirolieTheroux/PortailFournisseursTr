    ...
    debounceTimer = setTimeout(() => {
        fetch(`/services?search=${encodeURIComponent(searchTerm)}&offset=${offset}&limit=${limit}`)
            ...
            .then(data => {
                ...
                //resultCount.innerHTML = `${curentCount} sur ${totalCount} resultat(s)`;

                data.services.forEach(service => {
                    if(window.location.pathname === "/suppliers" && suppliersProductsServicesCodes.includes(service.code) || window.location.pathname !== "/suppliers"){
